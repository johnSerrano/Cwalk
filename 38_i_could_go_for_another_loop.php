<?
$page_title = 'I Could Go For Another Loop';
$html_title = $page_title . ' | cwalk';
$instructions = 'Fill in with values which will cause the program to output "4567"';
$html_description = "$page_title: $instructions";
require_once('header.php');
?>
<div class="instructions">
    <p><?= $instructions ?></p>
</div>
<div class="ide">#include &#60;stdio.h&#62;

int main(void)
{
    int n;
    for ( n = <input type="text" id="input_0" class="input-small"/>; n &#60; <input type="text" id="input_0" class="input-small"/>; n--) {
        printf("%d", n);
    }
    printf("\n");
}
</div>
<div id="message"></div>
<div>
    <p>
        <a id="check" class="btn btn-lg btn-info" href="#" role="button" onclick="check();">Check Answer</a>
        <a id="continue" class="btn btn-lg btn-success" href="#" role="button" style="display:none;">Next Question</a>  
    </p>
</div>
<? require_once('footer.php'); ?>
